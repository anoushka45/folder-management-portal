                                ...
                                <?php if ($numImages > 0): ?>
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <div class="button-group d-none d-md-flex">
                                            ...
                                            <button type="button" name="reject_images" class="btn icon-btn p-2 reject-button" data-type="images">
                                ...
                                <?php if ($numVideos > 0): ?>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        <div class="button-group d-none d-md-flex">
                                            ...
                                            <button type="button" name="reject_videos" class="btn icon-btn p-2 reject-button" data-type="videos">
    <img src="images/reject.svg" alt="Reject" class="icon-image icon-responsive">
</button>
...
$(document).ready(function() {
    $('.reject-button').on('click', function() {
        var mediaType = $(this).data('type'); // Get the type of media (images or videos)
        var selectedItems = (mediaType === 'images') ? $('.imageCheckbox:checked').map(function() {
            return $(this).val();
        }).get() : $('.videoCheckbox:checked').map(function() {
            return $(this).val();
        }).get();
        ...
        if (confirm('Are you sure you want to reject the selected items?')) {
            $.ajax({
                type: 'POST',
                url: 'process_selected_images_admin.php?event_id=<?php echo $eventId; ?>', // Your processing script
                data: {
                    selected_items: (mediaType === 'images') ? selectedItems : [],
                    selected_videos: (mediaType === 'videos') ? selectedItems : [],
                    reject_images: (mediaType === 'images') ? true : false,
                    reject_videos: (mediaType === 'videos') ? true : false
                },
                success: function(response) {
                    // Handle success response (e.g., update UI, notify user, etc.)
                    alert('Items rejected successfully.');
                    location.reload(); // Reload the page to see updated statuses
                },
                error: function() {
                    // Handle error response
                    alert('Error while rejecting items. Please try again.');
                }
            });
        }
    });
